    ...
    def fitness(evolved_parameters: np.array) -> float:
        ...
        for _ in range(config["nb_growth_evals"]):
            ...
            if config["shared_intial_graph_bool"]:
                ...
                G = generate_initial_graph(cells_data, positions_data, config["initial_network_size"], distance_threshold)
            ...
            for growth_cycle_nb in range(config["number_of_growth_cycles"]):
                ...
                # Predict new nodes
                new_nodes_predictions = predict_coordinates(mlp_growth_model, embeddings_for_growth_model)
            ...
            if not animate_graph_growth:  # Here we don't want to render the environment if we are just animating the graph growth when calling visualise_graph()
                ...
                for _ in range(config["nb_episode_evals"]):
                    if config["environment"] == "SmallWorldNetwork":
                        episode_reward = small_world_ness_fitness(G=G, niter=5, nrand=10, seed=config["seed"], render=render)
                    else:
...
def train_model(config):
    ...
    if config["shared_intial_graph_bool"]:
        config["shared_intial_graph"] = generate_initial_graph(cells_data, positions_data, config["initial_network_size"], distance_threshold)
